...
use utils::git::GitRepo;

// 静态常量定义
const SCAFFOLD_DIR_NAME: &str = "scaffold";
const CODES_DIR_NAME: &str = "codes";
const TEMP_DIR_SUFFIX: &str = "-temp";
const SCAFFOLD_CONFIG_FILE: &str = "scaffold.json";
const HOME_SCAFFOLD_CONFIG: &str = ".scaffold.json";
const GITHUB_REPO_URL: &str = "https://github.com/casemcx/code-template";
...
impl UpdateHandler {
    ...
    pub fn execute(&self) -> Result<()> {
        ...
        // 1. 下载GitHub代码到临时目录
        let temp_dir = format!("{}/{}/{}{}", self.home_dir, SCAFFOLD_DIR_NAME, CODES_DIR_NAME, TEMP_DIR_SUFFIX);
        let target_dir = format!("{}/{}/{}", self.home_dir, SCAFFOLD_DIR_NAME, CODES_DIR_NAME);

        // 确保scaffold目录存在
        let scaffold_dir = format!("{}/{}", self.home_dir, SCAFFOLD_DIR_NAME);
        if !Path::new(&scaffold_dir).exists() {
            fs::create_dir_all(&scaffold_dir)
                .with_context(|| format!("创建{}目录失败: {}", SCAFFOLD_DIR_NAME, scaffold_dir))?;
        }
    ...
    fn write_scaffold_configs(&self, config_file: &str, configs: &[ScaffoldConfig]) -> Result<()> {
    ...
    fn clone_repository(&self, target_dir: &str) -> Result<()> {
        ...
        // 使用GitRepo克隆代码仓库
        let repo = GitRepo::new(GITHUB_REPO_URL);
    ...
    fn read_scaffold_config(&self, dir_path: &Path) -> Result<Option<ScaffoldConfig>> {
        let config_path = dir_path.join(SCAFFOLD_CONFIG_FILE);
        ...
        let config: ScaffoldConfig = serde_json::from_str(&content)
            .with_context(|| format!("解析{}失败: {}", SCAFFOLD_CONFIG_FILE, config_path.display()))?;
